-- Durable mobile sync receipts. The client supplies only a UUIDv4 mutation
-- id and typed evidence data; active company, driver, load and next state are
-- derived inside the existing own-driver authorization boundary.

create table public.driver_sync_receipts (
  actor_id uuid not null references auth.users(id) on delete restrict,
  client_mutation_id uuid not null,
  company_id uuid not null references public.companies(id) on delete restrict,
  load_id uuid not null,
  operation text not null check (operation in ('advance_state', 'record_evidence')),
...
alter table public.driver_sync_receipts enable row level security;
alter table public.driver_sync_receipts force row level security;
revoke all on table public.driver_sync_receipts from public, anon, authenticated;

create function public.advance_own_driver_load_state_idempotent(
...
create function public.record_own_driver_load_evidence_idempotent(
  client_mutation_id uuid,
